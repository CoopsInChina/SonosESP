...
void refreshDeviceList() {
    ...
    // Get the actual list width
    int list_width = lv_obj_get_width(list_devices);
    if (list_width <= 0) {
        list_width = SCALE(620);  // Fallback
    }
    ...
    for (int i = 0; i < cnt; i++) {
        ...
        // Create main button - leave room for container padding
        lv_obj_t* btn = lv_btn_create(list_devices);
        
        // Calculate width based on list width minus padding
        int btn_width = list_width - SCALE(16);  // 8px padding on each side
        int btn_height;
        ...
        if (hasGroup) {
            for (int j = 0; j < cnt; j++) {
                ...
                int mem_width = btn_width - SCALE(40);  // Indent by 40px
    ...
    for (int i = 0; i < cnt; i++) {
        ...
        if (!coordinatorFound) {
            ...
            lv_obj_t* btn = lv_btn_create(list_devices);
            int btn_width = list_width - SCALE(16);  // Account for container padding
